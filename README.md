...
只有用户主动把当前非首选改选为目标候选并成功提交，才会增加学习级别；
学习后已经成为首选的正常空格、标点、回车或点击不会继续强化，自动提前上屏也不强化。
同一编码与片段使用 10 级持久偏好：跨上下文第 1 级 +6，此后每级 +2，
第 10 级封顶 +24；原纠正上下文额外保留约 +3，对应 +9 到 +27。
学习没有时间衰减；只有后续人工纠正、撤销或清空会改变结果。同一上下文改选竞争候选时会压低旧偏好。
新规则使用新的学习模式命名空间；旧数据库记录保留，但旧版普通首选强化记录不会参与新模式打分。
...
2026-09-19 新的本地打包模型为 20% 原发布版 + 80% mohu 的融合裁剪版，
224,475,584 字节（214.08 MiB），SHA256：
`23216acd8319885aa2431ffbf2231dab4677c5d4abb55a08a404450a15b865ca`。
格式与文件名不变，仍使用 compact。两万条离线首选准确率为 99.260%，
原发布版为 99.000%；样本已参与模型选择，尚非独立确认。
此模型尚未上传 Release；旧附件不因源码更新而自动改变，请以附件校验值为准。
