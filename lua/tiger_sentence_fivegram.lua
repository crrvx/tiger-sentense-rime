...
local function parse_header(data, actual_size)
    ...
    assert(u32(data,9) == 1 and u32(data,13) == HEADER_SIZE, "unsupported TCSKNM03 version")
    ...
    for i=0,4 do
        local p=161+i*16
        result.quant[i+1] = {
            pmin=i32(data,p)/1e7, pstep=u32(data,p+4)/1e12,
            bmin=i32(data,p+8)/1e7, bstep=u32(data,p+12)/1e12
        }
    end
    return result
end
...
function M.load(path, limits)
    ...
    local ok, result = pcall(function()
        ...
        for id=0,header.vocab_count-1 do
            ...
            local pq=u16(vocab_raw,p); local bq=u16(vocab_raw,p+2); p=p+4
        ...
        local function lookup(order,history,start,target)
            ...
            local pos=1
            local block_header=context_len*2+4
            while pos<=#data do
                local compared=compare_block(data,pos,history,start,context_len)
                local bowq=u16(data,pos+context_len*2)
                local count=u16(data,pos+context_len*2+2)
                local successors=pos+block_header
                if compared==0 then
                    local lo,hi=0,count
                    while lo<hi do
                        local mid=math.floor((lo+hi)/2)
                        local value=u16(data,successors+mid*4)
                        if value<target then lo=mid+1 else hi=mid end
                    end
                    local bow=decode_backoff(bowq,header.quant[order-1])
                    if lo<count and u16(data,successors+lo*4)==target then
                        return decode_probability(u16(data,successors+lo*4+2),header.quant[order]),bow,true
                    end
                    return nil,bow,false
                elseif compared>0 then
                    return nil,0.0,false
                end
                pos=successors+count*4
            end
            return nil,0.0,false
        end
        ...
        local model={path=path,bytes=actual,format="TCSKNM03",order=5,bos_id=header.bos}
